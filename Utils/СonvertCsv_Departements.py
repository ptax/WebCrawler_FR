...
def SaveCsv():
    ListNone = []
    NameSaveFile = os.path.abspath('../WorkBaseFile/Departements_28_08_17.csv')
    HeaderLine = DataStructure.FirstColumHeader.GetHeader_Departements('\t')
    text_file = open(NameSaveFile, "a")
    text_file.write(HeaderLine + '\n')
    DictFile = Utils.SaveAndLoadDictFile.LoadDict('../WorkBaseFile/Departements_28_08_17_cards')
    print len(DictFile)
...
def ConvertCSVFirstData():
    NameSaveFile = os.path.abspath('../WorkBaseFile/Departements_28_08_17_cards.txt')
    HeaderLine = DataStructure.FirstColumHeader.GetHeader_Departements('\t')
    text_file = open(NameSaveFile, "a")
    text_file.write(HeaderLine + '\n')
    DictFile = Utils.SaveAndLoadDictFile.LoadDict('../WorkBaseFile/Departements_28_08_17_cards')
    ...
    for Data, Keys in zip(DictFile.values(), DictFile.keys()):
        I_Code_departament = str(Keys).strip()
        I_Region = Data['I_Region'].strip()
        I_Dep = Data['I_Dep'].strip()
        I_Chefleeu = Data['I_Chefleeu'].strip()
        I_Tncc = Data['I_Tncc'].strip()
        I_Ncc = Data['I_Ncc']
        I_Nccenr = Data['I_Nccenr'].strip()
        ...
        try:
            ...
            Wiki_Name_Snipet = 'None'
        try:
            Wiki_name_departement = Data['Wiki_name_departement'].replace('\n', ' ').strip()
        except:
            Wiki_name_departement = 'None'

        try:
            W_Pays = Data['W_Pays'].replace('\n', ' ')
        except:
            W_Pays = 'None'
        try:
            W_Region = Data['W_Region'].replace('\n', ' ')
        except:
            W_Region = 'None'
        try:
            W_Creation_du_departementt = Data['W_Creation_du_departementt'].replace('\n', ' ').strip()
        except:
            W_Creation_du_departementt = 'None'
        try:
            W_Chef_lieu = Data['W_Chef-lieu'].replace('\n', ' ').strip()
        except:
            W_Chef_lieu = 'None'
        try:
            W_Sous_pefectures = Data['W_Sous_pefectures'].replace('\n', ' ').strip()
        except:
            W_Sous_pefectures = 'None'
        try:
            W_President_du_conseil_departemental = Data['W_President_du_conseil_departemental'].replace('\n',
                                                                                                        ' ').strip()
        except:
            W_President_du_conseil_departemental = 'None'
        try:
            W_Prefet = Data['W_Prefet'].replace('\n', ' ').strip()
        except:
            W_Prefet = 'None'
        try:
            W_Code_Insee = Data['W_Code_Insee'].replace('\n', ' ').strip()
        except:
            W_Code_Insee = 'None'
        try:
            W_Code_ISO_3166_2 = Data['W_Code_ISO_3166-2'].replace('\n', ' ').strip()
        except:
            W_Code_ISO_3166_2 = 'None'
        try:
            W_Code_Eurostat_NUTS_3 = Data['W_Code_Eurostat_NUTS-3-2'].replace('\n', ' ').strip()
        except:
            W_Code_Eurostat_NUTS_3 = 'None'
        try:
            W_Gentile = Data['W_Gentile'].replace('\n', ' ').strip()
        except:
            W_Gentile = 'None'
        try:
            W_Population = Data['W_Population'].replace('\n', ' ').strip()
        except:
            W_Population = 'None'
        try:
            W_Densite = Data['W_Densite'].replace('\n', ' ').strip()
        except:
            W_Densite = 'None'
        try:
            W_Superficie = Data['W_Superficie'].replace('\n', ' ').strip()
        except:
            W_Superficie = 'None'
        try:
            W_Arrondissements = Data['W_Arrondissements'].replace('\n', ' ').strip()
        except:
            W_Arrondissements = 'None'
        try:
            W_Circonscriptions_legislatives = Data['W_Circonscriptions_legislatives'].replace('\n', ' ').strip()
        except:
            W_Circonscriptions_legislatives = 'None'
        try:
            W_Cantons = Data['W_Cantons'].replace('\n', ' ').strip()
        except:
            W_Cantons = 'None'
        try:
            W_Intercommunalites = Data['W_Intercommunalites'].replace('\n', ' ').strip()
        except:
            W_Intercommunalites = 'None'
        try:
        ...
        if W_Region:
            ...
            DataSave = [I_Code_departament,
                        ...
                        Wiki_Url,
                        Wiki_Name_Snipet,
                        Wiki_name_departement,
                        W_Pays,
                        W_Region,
                        W_Creation_du_departementt,
                        W_Chef_lieu,
                        W_Sous_pefectures,
                        W_President_du_conseil_departemental,
                        W_Prefet,
                        W_Code_Insee,
                        W_Code_ISO_3166_2,
                        W_Code_Eurostat_NUTS_3,
                        W_Gentile,
                        W_Population,
                        W_Densite,
                        W_Superficie,
                        W_Arrondissements,
                        W_Circonscriptions_legislatives,
                        W_Cantons,
                        W_Intercommunalites,
                        W_Communes,
                        W_Name_ru,
